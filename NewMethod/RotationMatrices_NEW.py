...
def rotate_around_z(input_vec,phi):
	...
	***Addition made to deal with new form/dimensions of input vector used by analytical method***
	"""
	...
	## First try method that works for old form of arrays - if this fails, use method that will handle new array form
	try:
		R =  np.array(R1 * np.matrix(input_vec_col))
	except:
		R = np.array(np.matmul(R1, input_vec)).T
